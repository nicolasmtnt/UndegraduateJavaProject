...
public class Marketplace {
     ...
     */
    public static void add(String type, String title, String year, int x, double price){
        ...
        try{
            ...
            map.put(uuid, new Number[]{(int)map.get(uuid)[1]+x,price});
        } catch (NullPointerException e){
            map.put(uuid, new Number[]{(int)x, (double)price});
        }
    }
    /**
    ...
    public static void substract(String type, String title, String year, int x){
        UUID uuid = Items.getUUID(type, title, year);
        try{
            map.put(uuid, new Number[]{(int)map.get(uuid)[0]-x, map.get(uuid)[1]});
     ...
     * @param x
     */
    public static void toStock(String type, String title, String year, int x){
        UUID uuid = Items.getUUID(type, title, year);
